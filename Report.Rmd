...
For `bedroom`, `price`'s distribution slightly shifts upwards by a few hundred dollars for each additional bedroom. For `bathroom`, the distribution also moves upwards with the biggest drop from 2 to 2.5 bathrooms and another upwards shift in the distribution from 2.5 to 3 bathrooms. For `den`, the distribution is roughly equal for both groups with a difference-between-means of only \$87/month. This means that, in terms of potential modeling power, `bathroom` may be better than `bedroom`, and it may be better not to use `den`. TODO: comment on the boxplot overlaps maybe being an issue

\newpage

We're also curious about the correlations between the variables so we plot the upper triangle of a (Pearson) correlation matrix.
...
The variables with the strongest linear correlation with `price` are `bathroom` (r = 0.47) then `bedroom` (r = 0.39), though these are only moderately strong. In addition to the evidence from the boxplots, this further confirms that `den` is weakly correlated with `price` at r = 0.05. Moreoever, `bedroom` and `bathroom` are moderately correlated with each other at r = 0.63, which may indicate that they're collinear, which may affect our modeling decisions later on.

In addition, here are frequency tables between `high_price` and `bedroom` and `bathroom`, excluding `den` because we've already seen that it's a weak predictor. TODO: fix column names
...
The key take-aways of this exploratory analysis are:

- TODO: outliers removed
- TODO: no class imbalance for `high_price`
- TODO: `den` has the weakest linear correlation with `price` as show by the boxplots and correlation metric; discuss the other dropped predictors
