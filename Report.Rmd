...
Another way of checking the distribution of the error is a Q-Q plot. If the error is normally distributed, then a Q-Q plot will show all data points distributed along a straight, sloped line wherein x = y.
...
The Q-Q plot of the model shows the data following this line for the most part, but the points diverge at the beginning and the end, implying that the model has more extreme data than can be expected under a true normal distribution (Clay, 2015). Applying transformations to the response variable, such as a log transformation and a square root transformation, were unsuccessful in creating a more favourable distribution.
...
tidy(logistic_add, conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) %>%
  ...
  kable()
```
The first table for the non-interaction model models log-odds whereas the second table models odds.

Like our linear model, we cannot conclude that there is significant interaction between the predictors at a 0.05 significance level, so we proceed with the model without interaction.
...
An additional bedroom increases the expected *log-odds* of a property having high rent by 0.05 and increases the expected *odds* of a property having high rent by $e^{0.05}$ = 1.05 or 5%. However, the p-value of 0.21 for this estimate implies that this conclusion is not statistically significant at a 0.05 significance level. Similarly, an additional bathroom increases the odds of a property having high rent by $e^{0.36}$ = 1.43 or 43%, which is statistically significant.

Again, we do some model diagnostics.

One of the first things to check for is overdispersion, which occurs when our response distribution deviates from the assumption that the distribution of the fitted values is consistent with a Bernoulli distribution. The model has a dispersion parameter of 0.22, implying insignificant levels of overdispersion.

```{r, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
# Overdispersion
logistic_add$deviance / logistic_add$df.residual

# Precision, accuracy, recall, sensitivity
# TODO
```

\newpage

As an additional assessment, we use a binned residuals plot, where most data points (~95%) should fall within the generated error bounds for the model to be considered appropriate (Ludecke et al., n.d.).
