...
Austin did most of the modeling section...

Olivia did most of the exploratory data analysis section...

Raina wrote the introduction and data description...

Woobin wrote the conclusion...
...
Now that we have explored the data, we can consider models that can predict rental price based on other variables. In the previous section, we narrowed down the variables we can use as predictors to the number of bedrooms and bathrooms. To use these predictors, we have two options: a multiple linear regression and a multiple logistic regression.

### Multiple Linear Regression

A multiple linear regression model generates predictions as a straight line with an intercept and at least one slope.

When making a multiple linear regression model, we often consider whether or not interaction exists between covariates. If there is interaction, then the slope of the prediction line will differ depending on the value of the covariates; if not the slope is constant. This can be checked with hypothesis testing by creating a model with interaction in R and checking the model's statistics.
...
As shown in the tables above, the interaction term (`bedroom:bathroom`) in the table produced by the model with interaction is reporting p-values in excess of our pre-decided level of significance (95%, necessitating a value below 0.05). Thus, we cannot confidently conclusively identify any interaction between the predictors and must proceed with the model without interaction.
...
To check if our linear model is appropriate, we run model diagnostics in the form of a residual and Q-Q plot.
