...
Olivia (group leader) did most of the exploratory data analysis section.
...
# Seeing the total number of rows
nrow(data)
...
The error appear to have a mean of 0 because the points appear evenly around both sides of e = 0, but the variance is larger at certain areas (like when the fitted values are around 2000 and 3000), so the variance is not perfectly consistent.

\newpage

A Q-Q plot is another way to check the errors' distribution. If the error is normally distributed, then a Q-Q plot will show all data points distributed along a straight, sloped line wherein x = y.
...
A multiple logistic regression model is used to make predictions for a binary response variable so we made `price` binary by splitting it into two categories: above or below the mean. So our model will predict whether a property has above or below average rent, rather than a specific value.
...
Note that the first table for the non-interaction model models log-odds and the second models odds.
...
The `estimate` column in the first table for the non-interaction model shows a prediction line of the form:

$$\log\left(\frac{\hat{p}}{1-\hat{p}}\right) = -0.04 + 0.05 \cdot \text{bedroom} + 0.36 \cdot \text{bathroom}$$

where $\hat{p}$ is the expected probability of a property having high rent and $\frac{\hat{p}}{1-\hat{p}}$ is its odds.

An additional bedroom increases the expected *log-odds* of a property having high rent by 0.05 and increases the expected *odds* of a property having high rent by a factor of $e^{0.05}$ = 1.05 or 5%. However, the p-value of 0.21 for this estimate implies that this conclusion is not statistically significant at a 0.05 significance level. Similarly, an additional bathroom increases the odds of a property having high rent by $e^{0.36}$ = 1.43 or 43%, which is statistically significant.
...
This means that each additional bedroom increases the expected *log-odds* of a property having high rent by 0.05 and increases the expected *odds* of a property having high rent by a factor of $e^{0.05}$ = 1.05 or 5%. However, the p-value of 0.21 for this estimate implies that this conclusion is not statistically significant at a 0.05 significance level. Similarly, an additional bathroom increases the odds of a property having high rent by a factor of $e^{0.36}$ = 1.43 or 43%, which is statistically significant.

Again, we do some model diagnostics.

One of the first things to check for is overdispersion, which occurs when our response distribution deviates from the assumption that the distribution of the fitted values is consistent with a Bernoulli distribution. The model has a dispersion parameter of 1.02, implying insignificant levels of overdispersion.
...
As an additional assessment, we use a binned residuals plot, where most data points (~95%) should fall within the generated error bounds for the model to be considered appropriate (Lüdecke et al., n.d.).

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4}
# Binned residuals plot
plot(binned_residuals(logistic_add_disp)) + 
  theme_classic()
```

Each dot is the average residual for one bin of predicted probabilities and the vertical lines show the uncertainty around that average residual. The gray lines show the acceptable range for residuals. Since 2 of the 6 bins fall outside the range, specifically at the estimated probabilities exceeding 80%, the plot shows that the model fits decently well and that higher predicted probabilities should be more carefully interpreted.
...
However, as seen from the residual plot and the Q-Q plot, the key assumptions of homoscedasticity and the normality of residuals for the linear model are not completely valid. These violations affect prediction confidence intervals and the p-values. Namely, the current MLR model results cannot be considered fully reliable. Consequently, findings regarding property characteristics should be interpreted with caution given potential omitted variable bias.

To improve model reliability in future research, two key adjustments are recommended:

\vspace{-0.05in}
- Re-introducing location through neighbourhood clusters may help reduce unexplained variance and standard errors, thereby addressing potential omitted variable bias.
- Collecting continuous structural variables like square footage alongside discrete room counts may help normalize the residual distribution.

Standard 95% confidence thresholds should be maintained to ensure hypothesis testing remains statistically unbiased.
